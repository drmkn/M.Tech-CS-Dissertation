...
class ANN(nn.Module):
    def __init__(self, input_dim, hidden_layers, n_class = 2, activation = 'relu',classification = True,bias = True):
        super().__init__()
...
if __name__ == "__main__":
    ...
    model = ANN(input_dim=config['num_features'],hidden_layers=config['hidden_features_ann'],
                classification=True)
    # print(torch.rand((10,config['num_features'])))
    print(model.predict_with_logits(x = torch.rand((10,config['num_features']))))
